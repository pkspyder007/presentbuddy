        ...
        <div className="flex-1 min-w-0">
          {customWallpaper ? (
            <div className={`flex items-center gap-2 text-xs transition-all duration-300 ${
              showSuccess ? 'text-green-600 dark:text-green-400' : 'text-slate-600 dark:text-slate-300'
            }`}>
              <svg className={`w-4 h-4 flex-shrink-0 transition-colors ${
                showSuccess ? 'text-green-500 dark:text-green-400' : 'text-slate-400 dark:text-slate-500'
              }`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              <span className="truncate font-medium">{customWallpaper}</span>
              {showSuccess && (
                <span className="ml-1 text-green-500 dark:text-green-400 animate-fade-in">✓ Saved</span>
              )}
            </div>
          ) : (
